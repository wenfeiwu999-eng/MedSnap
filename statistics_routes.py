...
# _get_db removed, using config.get_db
...
def stats_import_records():
    ...
    try:
        ...
        if record_ids:
            ...
            c.execute(f'SELECT extracted_data FROM medical_records WHERE id IN ({placeholders}) AND user_id=?', record_ids + [get_current_user_id()])
        elif dept_id:
            ...
            c.execute(f'SELECT extracted_data FROM medical_records WHERE role_id IN ({placeholders}) AND user_id=?', target_ids + [get_current_user_id()])
        else:
            c.execute('SELECT extracted_data FROM medical_records WHERE user_id=?', (get_current_user_id(),))
...
def stats_records_list():
    ...
    try:
        ...
        c.execute('SELECT id, case_number, role_id, template_id, original_filename, create_time '
                  'FROM medical_records WHERE user_id=? ORDER BY create_time DESC', (get_current_user_id(),))
        rows = [dict(r) for r in c.fetchall()]
        conn.close()
        # 为每条记录附加科室中文名
        for row in rows:
