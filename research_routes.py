    ...
    try:
        ...
        c.execute('''INSERT INTO research_results
            (result_id, data_type, dept_id, source_record_id, title, summary,
             core_metrics, conclusion, notes, status, create_time, update_time, user_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (result_id,
             data.get('data_type', ''),
...
def get_result(result_id):
    ...
    try:
        conn = get_db()
        c = conn.cursor()
        row = c.execute("SELECT * FROM research_results WHERE result_id = ? AND user_id = ?", (result_id, get_current_user_id())).fetchone()
        conn.close()
...
def update_result(result_id):
    ...
    try:
        ...
        # 检查是否存在
        existing = c.execute("SELECT result_id FROM research_results WHERE result_id = ? AND user_id = ?", (result_id, get_current_user_id())).fetchone()
        if not existing:
            conn.close()
            return jsonify({"status": "error", "msg": "成果不存在"})
        ...
        sql = f"UPDATE research_results SET {', '.join(update_fields)} WHERE result_id = ? AND user_id = ?"
...
def delete_result(result_id):
    ...
    try:
        conn = get_db()
        c = conn.cursor()
        c.execute("DELETE FROM research_results WHERE result_id = ? AND user_id = ?", (result_id, get_current_user_id()))
        deleted = c.rowcount
        conn.commit()
        conn.close()
...
def export_results():
    ...
    try:
        ...
        rows = c.execute(
            f"SELECT * FROM research_results WHERE result_id IN ({placeholders}) AND user_id = ? ORDER BY create_time DESC",
            result_ids + [get_current_user_id()]
        ).fetchall()
        conn.close()
