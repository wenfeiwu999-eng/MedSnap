...
def init_db():
    ...
    for col in ['role_id', 'template_id', 'extracted_data', 'confidence_data',
                ...
                'module_type', 'text_source', 'analysis_type', 'batch_id']:
...
def _process_audio_file(audio_path, filename, role_id, template_id,
                        ai_prompt, template_name, display_layout, batch_id=None):
    ...
    try:
        ...
        c.execute('''INSERT INTO medical_records
            (id, case_number, original_filename, role_id, template_id,
             extracted_data, confidence_data, raw_text, create_time,
             source_type, audio_transcript, qualitative_data, module_type, batch_id)
...
def _process_image_file(file_path, filename, role_id, template_id,
                        ai_prompt, template_name, display_layout,
                        module_type='image_ocr', batch_id=None):
                ...
                for img_path in image_paths:
                    try:
                        ...
                        c.execute('''INSERT INTO medical_records
                            (id, case_number, original_filename, role_id, template_id,
                             extracted_data, confidence_data, raw_text, create_time,
                             source_type, module_type, batch_id)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'image', ?, ?)''',
                            (record_id, case_number, filename, role_id, template_id,
                             json.dumps(data, ensure_ascii=False),
                             json.dumps(confidence_data, ensure_ascii=False),
                             raw_text, create_time, module_type or 'image_ocr', batch_id))
                        conn.commit()
                        conn.close()
                        results.append({
...
def _process_text_content(text_content, role_id, template_id,
    ...
    try:
        ...
        c.execute('''INSERT INTO medical_records
            (id, case_number, original_filename, role_id, template_id,
             extracted_data, confidence_data, raw_text, create_time,
             source_type, module_type, text_source, batch_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'text', 'text_input', ?, ?)''',
            (record_id, case_number, filename, role_id, template_id,
             json.dumps(data, ensure_ascii=False),
             json.dumps(confidence_data, ensure_ascii=False),
             raw_text, create_time, processed_text, batch_id))
        conn.commit()
        conn.close()
        ...
        try:
            if is_audio_file(file.filename):
                ...
                img_results, img_errors = _process_image_file(
                    file_path, file.filename, role_id, template_id,
                    ai_prompt, template_name, display_layout,
                    module_type='image_ocr', batch_id=batch_id)
                results.extend(img_results)
                errors.extend(img_errors)
            elif is_text_file(file.filename):
                # 文本文件处理
...
@app.route('/batch_export/<batch_id>', methods=['GET'])
def batch_export(batch_id):
    """导出指定批次的所有记录为统一Excel"""
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT * FROM medical_records WHERE batch_id=? ORDER BY create_time', (batch_id,))
    rows = c.fetchall()
    conn.close()
